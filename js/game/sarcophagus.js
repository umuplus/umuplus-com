function Sarcophagus() {
    this.dim = [128, 128];
    this.img = new Image();
    this.img.src = '/img/game/sarcophagus.png';
    this.img.onload = this.__render.bind(this);
}

Sarcophagus.prototype.setClickHandler = function (fn) {
    this.clickHandler = fn;
}

Sarcophagus.prototype.onReady = function (fn) {
    this.readyHandler = fn;
}

Sarcophagus.prototype.__render = function () {
    const self = this;
    this.el = $('<div />').css('position', 'absolute')
        .attr('id', 'sa_' + Math.random().toString().replace('0.', '').trim())
